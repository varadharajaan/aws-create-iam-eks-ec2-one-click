    ...
    for acc_idx, acc in enumerate(selected_accounts, 1):
        ...
        try:
            ...
            for region in REGIONS:
                ...
                # All your delete functions here...
                delete_custom_vpcs(region=region)
                delete_cloudwatch_alarms(region)
                delete_ec2_snapshots(region)
                delete_elastic_cache_clusters([],region)
                delete_api_gateway_rest_apis([],region)
                delete_all_iam_roles(region)
                delete_kms_keys([],region)
                delete_s3_buckets(BUCKETS_TO_KEEP, region)
                ...
                delete_auto_scaling_groups(region)
                delete_load_balancers_and_target_groups(region)
                delete_ebs_snapshots(region)
                delete_amis(region)
                delete_ebs_volumes(region)
                delete_lambda_functions(region)
                stop_codebuild_builds(region)
                delete_codebuild_projects(region)
                delete_codepipelines(region)
                purge_and_delete_sqs_queues(region)
                delete_sns_topics(region)
                delete_all_sns_subscriptions(region)
                delete_rds_instances(region)
                delete_route53_hosted_zones(region)
                delete_all_ecs_clusters(region)
                delete_elastic_beanstalk_applications(region)
                delete_launch_templates(region)
                delete_eks_clusters(region)
                delete_ecr_repositories(region)
                delete_codecommit_repositories(region)
                delete_codedeploy_applications(region)
                delete_multipart_uploads(region=region)
                delete_iam_roles(region=region)
                ...
                terminate_vpn_connections(region)
                delete_vpc_peering_connections(region)
                remove_vpc_endpoints(region)
